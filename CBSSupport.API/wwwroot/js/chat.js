...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const internalStaff = [
        ...
        { name: "Neewa Limbu", initials: "N", avatarClass: "avatar-bg-red" },
        { name: "Shreya Rai", initials: "S", avatarClass: "avatar-bg-blue" },
    ...
    const roleSwitcher = document.getElementById("role-switcher"),
        ...
        // ✅ FIX: Added the missing reference to the file preview container.
    ...
    const updateSendButtonState = () => sendButton.disabled = connection.state !== "Connected" || (messageInput.value.trim() === "" && (!fileInput || !fileInput.files.length));
    const scrollToBottom = (element) => element.scrollTop = element.scrollHeight;
    const getAvatarDetails = (userName) => teamMembers.find(u => u.name === userName) || customerList.find(u => u.name === userName) || internalStaff.find(u => u.name === userName) || { initials: userName.substring(0, 1).toUpperCase(), avatarClass: "avatar-bg-blue" };
    const generateGroupName = (u1, u2) => [u1, u2].sort().join('_');
    const getChatHistory = (id) => JSON.parse(localStorage.getItem(id)) || [];
    const saveMessageToHistory = (id, data) => { const history = getChatHistory(id); history.push(data); localStorage.setItem(id, JSON.stringify(history)); };
    ...
    // ✅ FIX: Re-instated the full file handling and preview logic.
    ...
    function displayFilePreview(file) {
        ...
        filePreviewContainer.innerHTML = `
            <div class="file-preview-item">
                ...
                <button type="button" id="remove-file-preview" class="btn-close" aria-label="Remove file"></button>
            </div>`;
    ...
    async function uploadFileAndSendMessage(file) {
        const maxFileSize = 10 * 1024 * 1024;
        if (file.size > maxFileSize) { alert(`Error: File size cannot exceed ${maxFileSize / 1024 / 1024} MB.`); removeFilePreview(); return; }
    ...
    function displayMessage(messageData, isHistory) {
        ...
        const readReceiptHtml = isSent ? `<span class="read-receipt fas fa-check-circle" title="Sent"></span>` : '';
        const senderHtml = isSent ? '' : `<div class="message-sender">${messageData.sender}</div>`;
        const avatarHtml = isSent ? '' : `<div class="avatar-initials ${avatar.avatarClass}" data-bs-toggle="tooltip" title="${messageData.sender}">${avatar.initials}</div>`;
        ...
        messageRow.innerHTML = `
            ...
            <div class="message-content">
                ...
                <div class="message-meta"><span class="message-timestamp">${formatTimestamp(messageData.timestamp)}</span>${readReceiptHtml}</div>
            </div>`;
    ...
    // ✅ FIX: Restored and verified the ticket/inquiry pop-up chat functionality.
    // --- POPUP CHAT LOGIC ---
    function displayInquiryChatMessage(messageData) {
        const isSent = messageData.sender === currentUserIdentity;
        const messageRow = document.createElement('div');
        messageRow.className = `message-row ${isSent ? 'sent' : 'received'}`;
        messageRow.innerHTML = `<div class="message-content"><div class="message-bubble"><p>${messageData.text}</p></div><div class="message-meta"><span class="message-timestamp">${messageData.sender} - ${formatTimestamp(messageData.timestamp)}</span></div></div>`;
    ...
    function sendInquiryChatMessage() {
        ...
        displayInquiryChatMessage(messageData); // Display immediately
    ...
    function sendTicketChatMessage() {
        ...
        displayTicketChatMessage(messageData); // Display immediately
        popupTicketChatInput.value = '';
        popupTicketChatInput.focus();
    }

    // --- DYNAMIC CHAT & PAGE MANAGEMENT ---
    function renderSidebar(role, isAdmin) {
        ...
        if (isAdmin) {
            ...
            if (role === supportAgentIdentity) {
                const customerItems = customerList.map(user => createChatItem('private', generateGroupName(role, user.name), user.name, 'Private Chat', 'fa-user')).join('');
            ...
            const allStaff = [...teamMembers, ...internalStaff];
            const teamItems = allStaff.filter(u => u.name !== role).map(user => createChatItem('private', generateGroupName(role, user.name), user.name, 'Private Chat', 'fa-user')).join('');
            ...
            html += createChatItem('private', generateGroupName(role, supportAgentIdentity), supportAgentIdentity, 'Private Chat', 'fa-headset');
        }
    ...
    async function switchChatContext(contextData) {
        ...
        if (contextData.type === 'private' || contextData.type === 'group') {
            await connection.invoke("JoinPrivateChat", contextData.id).catch(err => console.error(`Failed to join group ${contextData.id}:`, err));
        }
    }
    ...
    async function sendMessage() {
        ...
        // Stop broadcasting "is typing" immediately on send
        clearTimeout(typingTimeout);
        connection.invoke("UserStoppedTyping", currentChatContext.id, currentUserIdentity);

        if (file) { await uploadFileAndSendMessage(file); }
        else {
            try {
                const method = currentChatContext.type === 'group' ? "SendPublicMessage" : "SendPrivateMessage";
                const args = currentChatContext.type === 'group'
    ...
    messageInput.addEventListener("keyup", (e) => {
        ...
        if (currentChatContext.id) {
            connection.invoke("UserIsTyping", currentChatContext.id, currentUserIdentity);
            clearTimeout(typingTimeout);
            typingTimeout = setTimeout(() => { connection.invoke("UserStoppedTyping", currentChatContext.id, currentUserIdentity); }, 3000);
        }
    });

    // ✅ FIX: Re-wired the attachment button event listener.
    attachmentButton.addEventListener('click', () => fileInput.click());
    fileInput.addEventListener('change', () => { if (fileInput.files.length > 0) { displayFilePreview(fileInput.files[0]); } else { removeFilePreview(); } updateSendButtonState(); });

    // ✅ FIX: Verified pop-up chat event listeners are correct.
    ...
    connection.on("ReceivePublicMessage", (messageId, sender, msg, time, initials, fileUrl, fileName, fileType) => {
        const data = { id: messageId, sender, message: msg, timestamp: time, fileUrl, fileName, fileType, seenBy: [] };
    ...
    connection.on("MessageSeen", (messageId, userName, seenTime) => {
        let chatID = null;
        if (getChatHistory('public').find(m => m.id === messageId)) chatID = 'public';
        else chatID = currentChatContext.id;
        updateMessageInHistory(chatID, messageId, (message) => { if (message && !message.seenBy.some(u => u.name === userName)) { message.seenBy.push({ name: userName, time: seenTime }); } });
        const messageEl = document.getElementById(`msg-${messageId}`);
        if (messageEl && messageEl.closest('#chat-panel-body')) {
            const receiptEl = messageEl.querySelector('.read-receipt');
            if (receiptEl) {
                const message = getChatHistory(chatID).find(m => m.id === messageId);
                if (message) {
                    const seenByNames = message.seenBy.map(u => u.name).filter(name => name !== currentUserIdentity);
                    if (seenByNames.length > 0) {
                        const newTitle = `Seen by: ${seenByNames.join(', ')}`;
                        ...
                        else { receiptEl.setAttribute('data-bs-original-title', newTitle); new bootstrap.Tooltip(receiptEl, { boundary: document.body }); }
                    }
                }
            }
        }
    ...
    connection.on("ReceiveTypingNotification", (groupName, userName, isTyping) => {
    ...
    // --- TICKET & INQUIRY SYSTEM LOGIC (Unchanged - already dynamic) ---
    ...
    const inquiriesTable = $('#inquiriesDataTable').DataTable({
        ...
        order: [[3, 'desc']], pageLength: 5, lengthChange: true, lengthMenu: [[5, 10, 20, -1], ['5', '10', '20', 'All']],
        dom: "<'row'<'col-sm-12 col-md-6'l><'col-sm-12 col-md-6'f>>" + "<'row'<'col-sm-12'tr>>" + "<'row'<'col-sm-12 col-md-5'i><'col-sm-12 col-md-7'p>>",
        language: { search: "", searchPlaceholder: "Search inquiries...", emptyTable: "No inquiries yet." },
        initComplete: function () { $(this.api().table().container()).find('.dataTables_filter input').before('<i class="fas fa-search search-icon"></i>'); }
    });
    ...
    const ticketsTable = $('#supportTicketsDataTable').DataTable({
        ...
        columns: [
            ...
            { data: 'createdBy', defaultContent: 'N/A' }, { data: 'resolvedBy', defaultContent: 'Pending' },
            { data: 'status', render: generateStatusBadge, defaultContent: '' }, { data: 'priority', render: generatePriorityBadge, defaultContent: 'Low' },
            { data: 'id', orderable: false, searchable: false, render: function (data, type, row) { const chatButton = `<button class="btn btn-sm btn-success start-ticket-popup-chat-btn ms-1" data-ticket-id="${row.id}" data-ticket-subject="${row.subject}" aria-label="Chat about ticket ${row.id}" title="Chat about this ticket"><i class="fas fa-comments"></i></button>`; const viewDetailsButton = `<button class="btn btn-sm btn-info view-details-btn" data-ticket-id="${data}" title="View Details"><i class="fas fa-eye"></i></button>`; return `<div class="d-flex">${viewDetailsButton}${chatButton}</div>`; } }
        ],
        order: [[2, 'desc']], pageLength: 5, lengthChange: true, lengthMenu: [[5, 10, 20, -1], ['5', '10', '20', 'All']],
        dom: "<'row'<'col-sm-12 col-md-6'l><'col-sm-12 col-md-6'f>>" + "<'row'<'col-sm-12'tr>>" + "<'row'<'col-sm-12 col-md-5'i><'col-sm-12 col-md-7'p>>",
    ...
    $(supportTicketForm).on('submit', function (e) {
        ...
        try {
            const now = new Date();
            const newTicket = { id: String(Date.now()).slice(-6), subject: $('#ticketSubject').val(), submissionTimestamp: now.toISOString(), createdBy: $('#fullName').val() || 'System', resolvedBy: 'Pending', description: $('#ticketDescription').val(), remarks: $('#ticketRemarks').val() || 'N/A', status: 'Pending', priority: $('#ticketPriority').val(), expiryDate: new Date($('#ticketExpiryDate').val()).toLocaleString(), assignedTo: 'Unassigned', lastUpdate: now.toISOString() };
            const allTickets = getTickets(); allTickets.unshift(newTicket); localStorage.setItem(ticketsLocalStorageKey, JSON.stringify(allTickets));
            if (newTicket.description) { saveTicketChatMessage(newTicket.id, { sender: newTicket.createdBy, text: `Ticket Created: ${newTicket.description}`, timestamp: newTicket.submissionTimestamp }); }
            ticketsTable.clear().rows.add(getFilteredTickets()).draw(); this.reset(); $(this).removeClass('was-validated'); newTicketModal.hide(); alert('Support ticket submitted successfully!');
        } catch (error) { console.error("Error submitting ticket:", error); alert('An error occurred while submitting the ticket. Please check the console for details.'); }
    });
    ...
    $('#supportTicketsDataTable tbody').on('click', '.view-details-btn', function () {
        ...
        if (ticket) {
            $('#details-id').text(`#${ticket.id}`); $('#details-status').html(generateStatusBadge(ticket.status || 'Pending')); $('#details-priority').html(generatePriorityBadge(ticket.priority || 'Low'));
            $('#details-subject').text(ticket.subject || 'N/A'); $('#details-date').text(ticket.submissionTimestamp ? new Date(ticket.submissionTimestamp).toLocaleString() : 'N/A');
            $('#details-expiryDate').text(ticket.expiryDate || 'N/A'); $('#details-createdBy').text(ticket.createdBy || 'N/A'); $('#details-resolvedBy').text(ticket.resolvedBy || 'N/A');
            $('#details-description').text(ticket.description || 'N/A'); $('#details-remarks').text(ticket.remarks || 'N/A'); $('#edit-ticketId').val(ticket.id);
            $('#edit-fullName').val(ticket.createdBy || ''); $('#edit-ticketSubject').val(ticket.subject || ''); $('#edit-ticketPriority').val(ticket.priority || 'Low');
            ...
            $('#edit-ticketExpiryDate').val(expiryForInput); $('#edit-ticketDescription').val(ticket.description || ''); $('#edit-ticketRemarks').val(ticket.remarks || '');
            setViewModalMode('view'); viewTicketModal.show();
        }
    });
    $('#supportTicketsDataTable tbody').on('click', '.start-ticket-popup-chat-btn', function (e) { e.stopPropagation(); const ticketId = $(this).data('ticket-id').toString(); const ticketSubject = $(this).data('ticket-subject'); openTicketChat(ticketId, ticketSubject); });
    ...
    $(editTicketForm).on('submit', function (e) {
        ...
        if (ticketIndex > -1) {
            allTickets[ticketIndex].createdBy = $('#edit-fullName').val(); allTickets[ticketIndex].subject = $('#edit-ticketSubject').val(); allTickets[ticketIndex].description = $('#edit-ticketDescription').val();
            allTickets[ticketIndex].remarks = $('#edit-ticketRemarks').val() || 'N/A'; allTickets[ticketIndex].priority = $('#edit-ticketPriority').val();
            allTickets[ticketIndex].expiryDate = new Date($('#edit-ticketExpiryDate').val()).toLocaleString(); allTickets[ticketIndex].lastUpdate = new Date().toISOString();
            localStorage.setItem(ticketsLocalStorageKey, JSON.stringify(allTickets)); ticketsTable.clear().rows.add(getFilteredTickets()).draw();
            $('#details-subject').text(allTickets[ticketIndex].subject); $('#details-createdBy').text(allTickets[ticketIndex].createdBy);
            $('#details-description').text(allTickets[ticketIndex].description); $('#details-remarks').text(allTickets[ticketIndex].remarks);
            $('#details-priority').html(generatePriorityBadge(allTickets[ticketIndex].priority)); $('#details-expiryDate').text(allTickets[ticketIndex].expiryDate);
        }
