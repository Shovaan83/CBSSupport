...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const pageInitializers = {
        ...
        chats: function () {
            ...
            // =================================================================
            // THIS IS THE FIX. The old function is replaced with this correct one.
            // =================================================================
            function displayChatMessage(messageData, isHistory) {
                ...
                let bubbleContent = `<p class="message-text">${messageData.message || ''}</p>`;

                const messageHtml = `
                    ${!isSent ? `<div class="avatar-initials ${avatar.avatarClass}" title="${messageData.sender}">${avatar.initials}</div>` : ''}
                    <div class="message-content">
                        ${!isSent ? `<div class="message-sender small fw-bold">${messageData.sender}</div>` : ''}
        ...
        'ticket-management': function () {
            ...
            let ticketsTable = ticketsTableElement.DataTable({
                ...
                columns: [
                    ...
                    { data: 'status', render: generateStatusBadge },
                    { data: 'priority', render: generatePriorityBadge },
                    { data: 'lastUpdate', render: formatDateTime, defaultContent: '' },
                    {
                        data: null, orderable: false, searchable: false,
                        render: function (data, type, row) {
                            let resolveControl = (row.status !== 'Resolved')
                                ? `<div class="form-check form-check-inline me-2" title="Mark as Resolved"><input class="form-check-input mark-resolved-checkbox" type="checkbox" data-ticket-id="${row.id}"></div>`
                                : `<div class="text-success text-center me-2" title="Resolved"><i class="fas fa-check-circle"></i></div>`;
                            const chatButton = `<button class="btn btn-sm btn-primary start-ticket-chat-btn" data-ticket-id="${row.id}"><i class="fas fa-comments me-1"></i>Chat</button>`;
                            return `<div class="d-flex align-items-center justify-content-center">${resolveControl}${chatButton}</div>`;
                        }
                    }
                ],
                order: [[6, 'desc']], pageLength: 10, lengthChange: true, searching: true, info: true,
            ...
            function markTicketAsResolved(ticketId) {
                ...
                if (ticketIndex > -1) {
                    ...
                    allTickets[ticketIndex].resolvedBy = adminUserIdentity;
                    saveTickets(allTickets);
                    const rowNode = ticketsTable.rows().nodes().to$().filter(function () { return $(this).find('.start-ticket-chat-btn').data('ticket-id') == ticketId; });
                    if (rowNode.length > 0) { ticketsTable.row(rowNode).data(allTickets[ticketIndex]).draw(false); }
                }
            }

            function populateTicketDetails(ticketId) {
                const ticket = getTickets().find(t => t.id === ticketId.toString());
            ...
            function openChatBox(ticket) {
                ...
                chatBox.id = `chatbox-${ticket.id}`;
                chatBox.dataset.ticketId = ticket.id;
                ...
                renderChatMessagesInPopup(ticket.id, chatBody);
            }

            ticketsTableElement.on('click', 'tbody tr', function (e) { if ($(e.target).closest('.start-ticket-chat-btn').length || $(e.target).closest('.mark-resolved-checkbox').length) return; ticketsTable.$('tr.table-active').removeClass('table-active'); $(this).addClass('table-active'); const data = ticketsTable.row(this).data(); if (data) populateTicketDetails(data.id); });
            ticketsTableElement.on('click', '.start-ticket-chat-btn', function (e) { e.stopPropagation(); const ticketId = $(this).data('ticket-id').toString(); const ticket = getTickets().find(t => t.id === ticketId); if (ticket) openChatBox(ticket); });
            ...
            $(floatingChatContainer).on('click', e => {
                ...
                if (e.target.closest('.action-send')) {
                    ...
                    const ticketIndex = allTickets.findIndex(t => t.id === ticketId);
                    if (ticketIndex > -1) {
                        allTickets[ticketIndex].lastUpdate = new Date().toISOString();
                        saveTickets(allTickets);
                        const rowNode = ticketsTable.rows().nodes().to$().filter(function () { return $(this).find('.start-ticket-chat-btn').data('ticket-id') == ticketId; });
                        if (rowNode.length > 0) { ticketsTable.row(rowNode).data(allTickets[ticketIndex]).draw(false); }
                    }
            ...
            $(floatingChatContainer).on('keyup', 'textarea', function (e) { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); $(this).closest('.chat-box-footer').find('.action-send').click(); } });

            ticketsTableElement.on('change', '.mark-resolved-checkbox', function () { const ticketId = $(this).data('ticket-id').toString(); if (confirm("Are you sure you want to mark this ticket as Resolved?")) { markTicketAsResolved(ticketId); } else { $(this).prop('checked', false); } });
            ...
            $('#filter-status').append(statuses.map(s => `<option value="${s}">${s}</option>`)); $('#filter-priority').append(priorities.map(p => `<option value="${p}">${p}</option>`)); $('#filter-agent').append(agents.filter(a => a !== 'Unassigned').map(a => `<option value="${a}">${a}</option>`));
        ...
        'inquiry-management': function () {
            ...
            $.fn.dataTable.ext.search.push(function (settings, data, dataIndex) { if (settings.nTable.id !== 'inquiriesDataTable') return true; const selectedStatus = inquiryStatusFilter.value; if (selectedStatus === "") return true; const rowData = inquiriesTable.row(dataIndex).data(); return selectedStatus === (rowData.outcome || 'Pending'); });

            $('#inquiriesDataTable tbody').on('click', 'tr', function (event) { if ($(event.target).is('input.mark-completed-checkbox')) { return; } inquiriesTable.$('tr.table-active').removeClass('table-active'); $(this).addClass('table-active'); const rowData = inquiriesTable.row(this).data(); if (rowData) { populateInquiryChat(rowData.id); } });
            $('#inquiriesDataTable tbody').on('change', '.mark-completed-checkbox', function () { const inquiryId = $(this).data('inquiry-id'); if (confirm("Are you sure you want to mark this inquiry as completed?")) { markAsCompleted(inquiryId); } else { $(this).prop('checked', false); } });
