...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- STATE AND CONFIGURATION (Unchanged) ---
    ...
    // --- LOCALSTORAGE & SIGNALR (Unchanged) ---
    ...
    // --- CHATS PAGE LOGIC (Unchanged - This section is left untouched) ---
    function initializeChatsPage() {
        // This entire section is correct and remains untouched.
        ...
        messageInput.addEventListener("keyup", (e) => { if (e.key === "Enter") sendChatMessage(); });
    }
    ...
    function initializeTicketManagementPage() {
        let ticketsTable = ticketsTableElement.DataTable({
            ...
            order: [[6, 'desc']],
            // MODIFICATION: Re-enabling the standard DataTables controls
            pageLength: 10,
            lengthChange: true, // This enables the "Show X entries" dropdown
            searching: true,
            info: true,
        ...
        // MODIFICATION: Replaced the old filter with the robust column().search() API
        $('#filter-status, #filter-priority, #filter-agent').on('change', function () {
            ...
            // Apply search to each column individually. `true, false` enables regex but disables smart search for exact matching.
            ...
            ticketsTable.draw(); // Redraw the table with all filters applied
        });
    ...
    function initializeEnquiryManagementPage() {
        ...
        $('#inquiriesDataTable').DataTable({
            data: inquiryList,
            columns: [
                { data: 'id' }, { data: 'topic' }, { data: 'inquiredBy' }, { data: 'date' },
    ...
    // --- MAIN INITIALIZATION & SIGNALR (Unchanged) ---
