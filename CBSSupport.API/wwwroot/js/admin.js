...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const pageInitializers = {
            ...
            function populateTicketDetails(ticketId) {
                ...
                if (!ticket.conversation) {
                    ...
                    if (ticket.description) {
                        ticket.conversation.push({ sender: ticket.createdBy, text: ticket.description, timestamp: ticket.submissionTimestamp, isNote: false });
                    }
                }
                if (ticket.conversation.length > 0) {
                    ticket.conversation.forEach(msg => {
                        ...
                        const noteClass = msg.isNote ? 'note-bubble' : '';
                        conversationView.innerHTML += `<div class="message-row ${messageClass}"><div class="message-content"><div class="message-bubble ${noteClass}"><p class="message-text">${msg.text}</p></div><span class="message-timestamp">${msg.sender} - ${formatDateTime(msg.timestamp)}</span></div></div>`;
                    });
                } else {
        ...
        'enquiry-management': function () {
            $('#inquiriesDataTable').DataTable({
                data: inquiryList,
                columns: [{ data: 'id' }, { data: 'topic' }, { data: 'inquiredBy' }, { data: 'date' }, { data: 'outcome', render: o => `<span class="badge bg-info">${o}</span>` }],
                pageLength: 10, lengthChange: true, searching: true, info: true, dom: "<'row'<'col-sm-12 col-md-6'l><'col-sm-12 col-md-6'f>>" + "<'row'<'col-sm-12'tr>>" + "<'row'<'col-sm-12 col-md-5'i><'col-sm-12 col-md-7'p>>"
            });
        }
    };
